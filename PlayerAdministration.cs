...
#pragma warning restore IDE0044, CS0649
        ...
        private class Cui
        {
            ...
            /// <returns>New object name</rethurns>
        ...
        private string GetPlaytime(string playerid)
        {
            return SecondsToElapsed((double)PlaytimeTracker?.Call<double>("GetPlayTime", playerid));
        }
        private string SecondsToElapsed(double time)
        {
        ...
        // Credit: The below code was reused from GrTeleport, to get a Human readable position
        List<GridObject> CreateGrid()
        {
            try
            {
                List<GridObject> retval = new List<GridObject>();

                var worldSize = (ConVar.Server.worldsize); //3000
                float offset = worldSize / 2; // 1500
                var gridWidth = (calgon * worldSize); // 20
                float step = worldSize / gridWidth; //149.99r
                string start = "";
                ...
                for (float zz = offset; zz > -offset; zz -= step)
                {
                    ...
                    {
                        retval.Add(new GridObject { position = new Vector3(xx, 0, zz), gridReference = $"{start}{letter}{number}" });
        ...
        string GetGridReference(Vector3 position)
        {
            try
            {
                ...
                GridObject nearestSpawn = null;

                foreach (var s in spawnGrid)
                {
                    if (Vector3.Distance(s.position, position) < distance)
                    {
                        distance = Vector3.Distance(s.position, position);
                        nearestSpawn = s;
                    }
                }
                ...
                {
                    if (position.z > nearestSpawn.position.z)
                        direction += "N";
                    else if (position.z < nearestSpawn.position.z)
                        direction += "S";
                    if (position.x < nearestSpawn.position.x)
                        direction += "E";
                    else if (position.x > nearestSpawn.position.x)
                        direction += "W";

                    var mesg = $"{distance.ToString("0.00")} meters {direction} of {nearestSpawn.gridReference}";
                    return mesg.Replace("{distance}", distance.ToString("0.00")).Replace("{direction}", direction).Replace("{gridreference}", nearestSpawn.gridReference);
                }
                return "I don't know.";
            }
            catch (Exception ex)
        ...
        private void AddUserPageInfoLabels(ref Cui aUIObj, string aParent, string aUiUserId, ulong aPlayerId, ref BasePlayer aPlayer)
        {
            ...
            if (VerifyPermission(aUiUserId, CPermDetailInfo))
            {
                ...
                aUIObj.AddLabel(
                    ...
                    string.Format(lang.GetMessage("Position Label Format", this, aUiUserId), GetGridReference(aPlayer.ServerPosition)), string.Empty, 14, TextAnchor.MiddleLeft
                );
                aUIObj.AddLabel(
                ...
                {
                    aUIObj.AddLabel(
                        aParent, CUserPageLblPlaytimeLbAnchor, CUserPageLblPlaytimeRtAnchor, CuiColor.TextAlt,
                        string.Format(lang.GetMessage("Play Time Label Format", this, aUiUserId), GetPlaytime(aPlayerId.ToString())), string.Empty, 14,
                        TextAnchor.MiddleLeft
                    );
                }
        ...
        private static readonly CuiPoint CUserPageLblPlaytimeLbAnchor = new CuiPoint(0.025f, 0.465f);
        private static readonly CuiPoint CUserPageLblPlaytimeRtAnchor = new CuiPoint(0.975f, 0.505f);

        private static readonly CuiPoint CUserPageLblBalanceLbAnchor = new CuiPoint(0.025f, 0.42f);
        private static readonly CuiPoint CUserPageLblBalanceRtAnchor = new CuiPoint(0.975f, 0.46f);

        private static readonly CuiPoint CUserPageLblRewardPointsLbAnchor = new CuiPoint(0.025f, 0.375f);
        private static readonly CuiPoint CUserPageLblRewardPointsRtAnchor = new CuiPoint(0.975f, 0.415f);
        ...
        private List<GridObject> spawnGrid = new List<GridObject>();
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(
                new Dictionary<string, string>
                {
                    ...
                    { "Play Time Label Format", "Play time: {0}" },
