...
namespace Oxide.Plugins {
    [Info("Death Messages", "QUETE", "2.1.55")]
    class DeathMessages : RustPlugin {
        ...
        class PluginConfig {
            ...
            [JsonProperty("P. Nombre del Cientifio")]
        ...
        class Attacker {
            ...
            private string InitializeName() {
                ...
                switch (Type) {
                    ...
                    case AttackerType.Scientist:
                        return string.IsNullOrEmpty(Entity.ToPlayer()?.displayName) ? _config.ScientistName : Entity.ToPlayer()?.displayName;
        ...
        class DeathMessage {
            public DeathMessage(Attacker attacker, Victim victim, string weapon, string damageType, string bodyPart, double distance) {
                ...
                if (_config.Distance <= 0) {
                    ...
                    Players = new ListHashSet<BasePlayer>();
                    if (position != null) {
                        Players.Clear();
                        foreach (BasePlayer activePlayer in BasePlayer.activePlayerList) {
                            if (activePlayer.Distance((UnityEngine.Vector3)position) <= _config.Distance)
            ...
            private DeathReason InitializeReason() {
                ...
                else if (Attacker.Type == AttackerType.Guntrap)
                    return DeathReason.Guntrap;

                else if (Attacker.Type == AttackerType.Zombie)
                ...
                //              else if (Victim.Type == VictimType.Scientist)
                ...
                else if (Victim.Type == VictimType.Player || Victim.Type == VictimType.Scientist)
                    return GetDeathReason(DamageType);

                else if (Victim.Type == VictimType.Zombie)
            ...
            private string InitializeDeathMessage() {
                ...
                switch (Victim.Type) {
                    ...
                    case VictimType.Scientist:
                        victimName = _config.ScientistName;
                        break;
        ...
        private void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info) {
            ...
            if (entity.name.Contains("patrolhelicopter") && info.InitiatorPlayer != null)
        ...
        private void OnEntityDeath(BaseCombatEntity victim, HitInfo info) {
            ...
            if (!_config.ShowDeathAnimals && _victim.Type == VictimType.Scientist) {
                return;
            }
            if (!_config.ShowDeathAnimals && _victim.Type == VictimType.NPC) {
                return;
            }
